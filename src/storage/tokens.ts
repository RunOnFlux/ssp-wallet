...
async function sepolia() {
    ...
    let customTokens : any = await localForage.getItem(`custom-token-sepolia`);
    if (customTokens) {
...
async function eth() {
    ...
    let customTokens : any = await localForage.getItem(`custom-token-eth`);
    if (customTokens) {
      return tokens.concat(customTokens.slice())
    } else {
      return tokens;
