...
describe('Balances Lib', () => {
  describe('Verifies balances', () => {
    ...
    it('should return fetchAddressBalance data when value is flux', async () => {
        const res = await fetchAddressBalance('t1ex3ZyD2gYqztumQpgG6uPDGK5iHFY6aEd','flux');
    ...
    it('should return fetchAddressBalance data when value is blockbook type', async () => {
      const res = await fetchAddressBalance('bitcoincash:qzq4ehw7h3jgcx5tx687zyunfk6pm9hcrys4u3tvhl','bch');
      expect(res).to.not.be.null;
      expect(res).to.not.be.undefined;
      expect(res.unconfirmed).to.not.be.null;
      expect(res.unconfirmed).to.not.be.undefined;
      expect(res.totalTransactions).to.not.be.null;
      expect(res.totalTransactions).to.not.be.undefined;
      assert.equal(res.address, 'bitcoincash:qzq4ehw7h3jgcx5tx687zyunfk6pm9hcrys4u3tvhl');
    });

    it('should return fetchAddressBalance data when value is evm type', async () => {
      const res = await fetchAddressBalance('0x8092557902BA4dE6f83a7E27e14b8F0bF8ADFeA1','sepolia');
      expect(res).to.not.be.null;
      expect(res).to.not.be.undefined;
      expect(res.unconfirmed).to.not.be.null;
      expect(res.unconfirmed).to.not.be.undefined;
    ...
    it('should return fetchAddressTokenBalances data when value is invalid', async () => {
      await fetchAddressTokenBalances('t1ex3ZyD2gYqztumQpgG6uPDGK5iHFY6aEd','flux', []).catch((r) => {
    ...
    it('should return fetchAddressTokenBalances data when value is evm type', async () => {
      const res = await fetchAddressTokenBalances('0x8092557902BA4dE6f83a7E27e14b8F0bF8ADFeA1','sepolia', ['0x8092557902BA4dE6f83a7E27e14b8F0bF8ADFeA1']);
